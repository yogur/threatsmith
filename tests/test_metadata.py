...
def test_generate_metadata_returns_all_required_fields():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert isinstance(metadata, ThreatSmithMetadata)

        # Check all required fields exist
        assert metadata.threatsmith_version
...
def test_generate_metadata_captures_user_objectives():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        objectives = {
            "business": "Find all authentication flaws",
            "security": "PCI-DSS",
        }
        metadata = generate_metadata(
            engine_name="claude-code",
...
def test_write_metadata_creates_json_file():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Verify file contains valid JSON matching the dataclass
...
def test_generate_metadata_no_objectives_defaults_to_empty_dict():
    """Test that omitting user_objectives defaults to an empty dict."""
    with patch("subprocess.run") as mock_run:
        mock_run.side_effect = [
            type("obj", (object,), {"stdout": "hash\n", "returncode": 0})(),
            type("obj", (object,), {"stdout": "br\n", "returncode": 0})(),
